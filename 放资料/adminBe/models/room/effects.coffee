...
# 覆盖 saveall
  reload: ({payload}, {put}) ->
    console.log '这里是 room model reload 调用Service'
    result = yield services.reload()

    if result?
      yield put
        type: type.roomSaveAll
        payload: result.results
      yield payload.success result.results
    else
      yield payload.fail 'room reload error'

    return

  fetch: ({payload}, {put}) ->
    console.log '这里是 room model fetch 调用Service'
    ...
    if result?
      ...
      yield payload.fail 'room fetch error'

    return

# 增一个 save
  create: ({payload}, {put}) ->
    console.log '这里是 room model create 调用Service'
    validate.exist payload,['roomType', 'belongToHouse']
    ...
    if result?
      ...
      yield payload.fail 'room create error'

    return

# 改 patch
  update: ({payload}, {put}) ->
    console.log '这里是 room model update 调用Service'
    validate.exist payload,['objectId']
    ...
    if result?
      ...
      yield payload.fail 'room update error'

    return

# 删 remove
  delete: ({payload}, {put}) ->
    console.log '这里是 room model delete 调用Service'
    exist action.payload,['objectId']

    result = yield services.delete payload.objectId

    if result?
      yield put
        type: type.roomRemove
        payload:
          objectId: payload.objectId
      yield payload.success payload.objectId
    else
      yield payload.fail 'room delete error'
